(function () {
  ...
  lastThreeFiles.push(files[files.length - 1])
  lastThreeFiles.push(files[files.length - 2])
  lastThreeFiles.push(files[files.length - 3])

  for (let i = 0; i < lastThreeFiles.length; i++) {
    // file path
    const pathToFile = `${srcPath}/posts/${lastThreeFiles[i]}`

    // read data from file and then render post
    const postData = ssg.getPostDataFromMarkdown(pathToFile)
    lastThreeImages.unshift(postData.attributes.coverImage)
    // console.log(lastThreeImages)
  }
  ssg.extendsMainCSS(lastThreeImages, srcPath)
